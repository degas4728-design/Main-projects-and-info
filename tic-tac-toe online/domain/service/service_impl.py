...
from domain.model.model import GameStatus
...
class Gamelogic(GameService):
    ...
    def is_end(self, field, game):
        if self._is_win(field, 1):
            for uuid, symbol in game.symbols.items():
                if symbol == 1:
                    winner_uuid = uuid
                    break
            game.status = GameStatus.PLAYER_WIN
            return "PLAYER_WIN"
        if self._is_win(field, 2):
            for uuid, symbol in game.symbols.items():
                if symbol == 2:
                    winner_uuid = uuid
                    break
